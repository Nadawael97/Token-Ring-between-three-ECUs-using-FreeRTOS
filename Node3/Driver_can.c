...
TaskHandle_t xSendDistanceHandle = 5;

xSemaphoreHandle can_SendSem;
xSemaphoreHandle can_SendDistSem;
...
CANInterruptHandler( void )
{
    ...
    else if(ui32Status == 5)
    {
        ...
        UARTprintf("interrupt clearedTransmission Distance.\n");

        /* 'Give' the semaphore to unblock the task. */
        xSemaphoreGiveFromISR(can_SendDistSem, &xHigherPriorityTaskWoken);
...
void CanInt(void)
{
    /*Intiliazation of the port clock and the IDs of the Objects that will be recieved and send */
        ...
        g_pui8Msg1[0U]=0;
        g_sCANMsgObject1.ui32MsgID = 0x3001;
        ...
        g_sCANMsgObject5.ui32MsgID = 0x2001;
        ...
        g_sCANMsgObject3.ui32MsgID = 0x1001;
        ...
        CANMessageSet(CAN0_BASE, 3, &g_sCANMsgObject5, MSG_OBJ_TYPE_RX);
        CANMessageSet(CAN0_BASE, 1, &g_sCANMsgObject1, MSG_OBJ_TYPE_TX);
        CANMessageSet(CAN0_BASE, 5, &g_sCANMsgObject3, MSG_OBJ_TYPE_TX);
...
static void
SendDistanceTask(void *pvParameters)
{
    while(1)
    {
        CANMessageSet(CAN0_BASE, 5, &g_sCANMsgObject5, MSG_OBJ_TYPE_TX);
        xSemaphoreTake(can_SendDistSem, 0);
        UARTprintf("semaphore is empty.\n");
    }
...
uint32_t
CANDistanceTaskInit(void)
{
    CanInt();

    if(xTaskCreate(SendDistanceTask, (const portCHAR *)"Token", 128, NULL,
                      6, &xSendDistanceHandle) != pdTRUE)
    {
        return(1);
    }
...
{
    g_pui8Msg5[0U] = Distance;
    g_sCANMsgObject5.pui8MsgData = g_pui8Msg5;
    UARTprintf("distance in %d", g_pui8Msg5[0U]);
    if(CANDistanceTaskInit() != 0)
    {
